% 多码头确定性环境下模型求解代码，与不确定环境下的对比，在染色体中将最后两段表示松弛的量去除
...
% 后续工作：
% 1、指标的计算还要添加。目前只能得到一个方案的示意图，但方案的相关指标还没有得到
% 2、然后将计划到港船舶数据集扩充，目前只有20艘船，得到的最优方案有很多，它们的目标函数值都一样
% 3、按照算法改进方向，将算法在进行修改，使其能有所不同
% 4、文字部分进行修改，指标进行画图、算法步骤进行说明等，这部分完成后可以先试着投出去
% 5、继续研究不确定性的融入（看相关论文），看能不能找到更合适的建模方法，若能找到，在进行论文的修改
% 6、大论文方向：继续将问题的约束等问题扩展，添加岸桥服务范围等新的实际约束，来提高论文的充实度，其他方面的约束还要看论文
% 7、后面进行大论文的书写

clear;
clc;

global cost_chidao sample g cost_work cost_diff N_m L_m d_m N_q v_m cost_tr N_0 M_0 W_0 L_0 x_0 C_0 Q_first_0 d_0 N_v L M_pre W_u W_d A ED C_min C_max draft best_position cost_delay popsize G_max  cost_stay;         %  A_sigma v_sigma

% 其他数据 %
% 岸桥干扰系数 %
g = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'B3:B3');

% 单位时间单位岸桥作业成本 %   这些成本常量可以不用作为全局变量，只需放到 fitness 函数中即可
cost_work = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'C3:C3');

% 单位时间非生产性停泊费用
cost_stay = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'E3:E3');

% 单位集装箱单位距离偏离成本 %
cost_diff = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'D3:D3');

cost_chidao = 100;  % readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'J3:J3');

% 码头数据 %
% 码头数量 %
N_m = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'A6:A6');

% 岸线长度 %
L_m = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'C6:C8')';

% 水深条件 %
d_m = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'D6:D8')';

% 岸桥数量 %
N_q = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'E6:E8')';

% 岸桥作业效率
v_m = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'F6:F8')';

% 码头间单位箱量转运成本 %
cost_tr = 2 .* readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'C11:E13');

% 已靠泊船舶 %
% 已靠泊船舶总数量 %
N_0 = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'A17:A17');

% 所靠泊码头 %
M_0 = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'C17:C24')';

% 剩余作业量 %
W_0 = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'D17:D24')';

% 船舶长度 %
L_0 = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'E17:E24')';

% 靠泊位置 %
x_0 = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'F17:F24')';

% 岸桥数量 %
C_0 = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'G17:G24')';

% 起始岸桥编号 %
Q_first_0 = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'H17:H24')';
...
% 待调度船舶 %
% 待调度船舶数量 %
N_v = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'A28:A28');

% 船舶长度 %
L = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'F28:F47')';

% 预分配码头 %
M_pre = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'C28:C47')';

% 待装箱量 %
W_u = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'D28:D47')';

% 待卸箱量 %
W_d = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'E28:E47')';

% 船舶预计到港时刻 % 期望值
A = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'G28:G47')';

% 船舶预计到港时刻标准差
% A_sigma = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'N28:N47')';

% 岸桥作业效率标准差
% v_sigma = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'O28:O47')';

% 期望离港时刻 %
ED = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'H28:H47')';

% 最小岸桥数量 %
C_min = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'I28:I47')';

% 最大岸桥数量 %
C_max = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'J28:J47')';

% 吃水深度 %
draft = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'K28:K47')';

% 偏好位置 %
best_position = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'L28:L47')';

% 单位时间离港延迟惩罚 %
cost_delay = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'M28:M47')';

% 样本数据 %  同码头成本系数，这些样本数据也可以直接放到 fitness 函数中
% 实际到达时间
sample{1, 1} = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'sample_9_1', 'Range', 'B4:U23');
% 岸桥实际作业效率
sample{1, 2} = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'sample_9_2', 'Range', 'B4:U23');

% 算法参数 %
% 种群规模 %
popsize = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'F3:F3');

% 最大迭代次数 %
G_max = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'G3:G3');
...
% p_4 = 0.3;    % 变异概率2
% 变异概率 %
p_0 = readmatrix('E:/Paper/me/data_20.xlsx', 'Sheet', 'data_9', 'Range', 'I3:I3');
...
%     fitness_value_mean_temp = mean(fitness_value);    % 平均适应度值
    ...
    % 至此，选择步骤已完成，下一步进行交叉
    pop_cross = zeros(popsize, size(pop_select, 2));
    pop_muta = zeros(popsize, size(pop_select, 2));
...
%         pop_muta(j+1, :) = mutation(pop_cross(j+1, :));
    ...
    Matrix_2 = sortrows([pop_repaired fitness_value' pro_all'], size(pop_repaired, 2) + 1);      % #########    要加上计算指标pro    #########
    
    % 然后将前10%个替换为父代最好的10%个个体
    Matrix_2(1 : 0.1*popsize, : ) = best_five;     % ################    这里也要加上    ############
...
%         pop_improved(r_vessel + 7*N_v) = randi([-20 * v_sigma(r_vessel), 20 * v_sigma(r_vessel) ], 1, 1)/10;
        pop_improved(r_vessel + 2*N_v) = randi([max(A(r_vessel) * 10, 0), round(ED(r_vessel)*10, 0)], 1, 1)/10;    % 修改到港时间      % + pop_improved(r_vessel + 6*N_v)*10
        ...
        pop_improved(r_vessel + 5*N_v) = round(pop_improved(r_vessel + 2*N_v) + (W_u(r_vessel) + W_d(r_vessel))/(pop_improved(r_vessel+3*N_v) * (v_m(pop_improved(r_vessel))) * g^(pop_improved(r_vessel+3*N_v) - 1)), 1); % 修改离港时刻        %  - pop_improved(r_vessel+7*N_v)
