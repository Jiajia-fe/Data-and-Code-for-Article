function pop_init = population_init ()
    ...
    global popsize N_v A C_min C_max W_u W_d v_m g   M_pre best_position  N_q L_m L  % A_sigma v_sigma
...
%             pop_init(i, j+7*N_v) = v_sigma(j);
            ...
            % 第六部分：离港时刻计算
            pop_init(i, j+5*N_v) = round(pop_init(i, j+2*N_v) +  (W_u(j) + W_d(j)) / (pop_init(i, j+3*N_v) * (v_m(pop_init(i, j))) * g^(pop_init(i, j+3*N_v) - 1)), 1);     % - pop_init(i, j+7*N_v)
