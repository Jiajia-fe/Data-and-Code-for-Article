...
function pop_repaired = repair (pop_init)
    ...
    for i = 1 : size(pop_repaired, 1) 
        ...
        for j = 1 : N_v
            ...
            % 将靠泊时间调整为其到达时间
            % 相应的离港时刻也进行更新
            pop_repaired(i, j + 2*N_v) = max(A(j), 0);    %  + pop_repaired(i ,j + 6*N_v)
            pop_repaired(i, j+5*N_v) = round(pop_repaired(i, j+2*N_v) + (W_u(j) + W_d(j)) / (pop_repaired(i, j+3*N_v) * (v_m(pop_init(i, j))) * g^(pop_repaired(i, j+3*N_v) - 1)), 1);      %  - pop_init(i, j+7*N_v)
        ...
        for k = 1 : N_m
            ...
            while size(vessel_ready{1, k}, 2) > 0 % 只要还存在未完成调度的船舶
                ...
                if size(useful_space{1, k}, 2) > 0 % 存在可用的岸线区间时，不需要修改靠泊时间
                    ...
                    % 更新离港时间 
                    vessel_ready{1, k}(3, 1) = round(vessel_ready{1, k}(2, 1) + (W_u(vessel_ready{1, k}(1, 1)) + W_d(vessel_ready{1, k}(1, 1))) / (vessel_ready{1, k}(6, 1) * (v_m(k)) * g^(vessel_ready{1, k}(6, 1) - 1)), 1); % 对当前调度船舶离港时刻进行更新        %  - vessel_ready{1, k}(9, 1)
                ...
                % 当不存在可用岸线区间时，需要将当前调度船舶的靠泊时间推迟到已靠泊船舶中最早离港时刻，然后对下一个待调度船舶进行调度
                    ...
                    % 离港时刻更新
                    vessel_ready{1, k}(3, 1) = round(vessel_ready{1, k}(2, 1) + (W_u(vessel_ready{1, k}(1, 1)) + W_d(vessel_ready{1, k}(1, 1))) / (vessel_ready{1, k}(6, 1) * (v_m(k)) * g^(vessel_ready{1, k}(6, 1) - 1)), 1);    %  - vessel_ready{1, k}(9, 1)
