...
function pop_muta = mutation (pop_cross, p_m)
    ...
    global N_v L_m L C_min C_max N_q W_u W_d v_m g  A ED  % A_sigma v_sigma 
...
%             pop_muta(j+7*N_v) = randi([-20 * v_sigma(j), 20 * v_sigma(j) ], 1, 1)/10;
            pop_muta(j+2*N_v) = randi([max(A(j) * 10, 0), round(ED(j)*10, 0)], 1, 1)/10; % 对靠泊时间进行变异    % + pop_muta(j + 6*N_v)*10
            ...
            pop_muta(j+5*N_v) = round(pop_muta(j + 2*N_v) + (W_u(j) + W_d(j))/(pop_muta(j+3*N_v) * (v_m(pop_muta(j))) * g^(pop_muta(j+3*N_v) - 1)), 1); % 对离港时刻进行更新    %  - pop_muta(j+7*N_v)
