...
%                 disp("---------  作业成本  ---------");
%                 disp(cost_work * pop_repaired(i, l + 3*N_v) * (sample{1, 3}(k, l) - max(pop_repaired(i, l + 2*N_v), sample{1, 1}(k, l))));
...
%                 disp("---------  转运成本  ---------");
%                 disp(cost_tr(M_pre(l), pop_repaired(i, l)) * W_u(l));
%                 disp(M_pre(l));
%                 disp(pop_repaired(i, l));
                object_value(i, k) = object_value(i, k) + cost_tr(M_pre(l), pop_repaired(i, l)) * W_u(l);    % 转运成本
%                 disp("---------  偏移成本  ---------");
%                 disp(abs(pop_repaired(i, l + N_v) - best_position(l)));
%                 disp(is_change_terminal(l));
%                 disp(cost_diff * (W_u(l) + W_d(l)) * abs(pop_repaired(i, l + N_v) - best_position(l)) * is_change_terminal(l));
